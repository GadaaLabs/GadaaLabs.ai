...
// ── Section parsing ───────────────────────────────────────────────────────────
// Parse AI-generated markdown into sections for structured PDF layout.
...
export function StakeholderReportPDF({ report, summary }: Props) {
  const date = new Date().toISOString().slice(0, 10);
  const sections = parseSections(report);

  return (
    <Document
      title={`Executive Brief — ${summary.fileName}`}
      author="GadaaLabs"
      creator="GadaaLabs"
    >
      <Page size="A4" style={S.page}>
        ...
        <View style={S.banner}>
          ...
          <Text style={S.bannerSub}>
            Prepared by GadaaLabs AI · {date} · Plain-language summary for leadership
          </Text>
        </View>
        ...
        {/* Parsed sections from AI output */}
        {sections.length > 0 ? (
          sections.map((sec, i) => (
          ...
          // Fallback: render as single block if no ## headings
